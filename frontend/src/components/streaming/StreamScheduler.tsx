...
const StreamScheduler: React.FC<StreamSchedulerProps> = ({
  ...
  const { data: scheduledStreamsData, isLoading } = useQuery({
    ...
    queryFn: () => api.streams.getScheduledStreams(),
    enabled: isOpen,
  });
  ...
  const scheduleStreamMutation = useMutation({
    mutationFn: (data: any) => {
      if (streamId) {
        return api.streams.scheduleStream(
          streamId,
      ...
      // If no streamId, create new stream first (would need to implement this)
      throw new Error('Stream ID required for scheduling');
    },
    onSuccess: (data) => {
  ...
  const cancelScheduleMutation = useMutation({
    mutationFn: (streamId: string) => api.streams.cancelSchedule(streamId),
    onSuccess: () => {
