...
const SubscriptionPanel: React.FC<SubscriptionPanelProps> = ({
  ...
  const { data: subscriptionData, isLoading } = useQuery({
    ...
    queryFn: () => api.streams.getSubscriptionStatus(streamerId),
    enabled: isOpen,
  });
  ...
  const subscribeMutation = useMutation({
    mutationFn: ({ tier, paymentMethod }: { tier: string; paymentMethod: string }) =>
      api.streams.subscribeToStreamer(streamerId, tier, paymentMethod),
    onSuccess: () => {
  ...
  const unsubscribeMutation = useMutation({
    mutationFn: () => api.streams.unsubscribeFromStreamer(streamerId),
    onSuccess: () => {
