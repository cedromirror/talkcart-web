...
const ShareDialog: React.FC<ShareDialogProps> = ({ open, onClose, post }) => {
  ...
  const shareWithFollowersMutation = useMutation({
    mutationFn: (data: { message: string }) =>
      api.posts.shareWithFollowers(post.id || post._id, data),
    onSuccess: () => {
  ...
  const shareWithUsersMutation = useMutation({
    mutationFn: (data: { userIds: string[]; message: string }) =>
      api.posts.shareWithUsers(post.id || post._id, data),
    onSuccess: () => {
      toast.success(`Post shared with ${selectedUsers.length} users!`);
      queryClient.invalidateQueries({ queryKey: ['posts'] });
      onClose();
    },
    onError: (error) => {
              ...
              <List sx={{ maxHeight: 200, overflow: 'auto' }}>
                {filteredFollowing.map((user: any) => (
                  <ListItem key={user.id || user._id} disablePadding>
                    <ListItemButton onClick={() => handleUserToggle(user.id || user._id)}>
                      <ListItemAvatar>
                        <UserAvatar
                          src={user.avatar}
