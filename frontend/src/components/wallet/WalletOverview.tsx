...
const WalletOverview: React.FC<WalletOverviewProps> = ({
  ...
  const { portfolio, loading, refreshing, refreshWallet } = useWallet();
            ...
            <Box>
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    <Typography variant="h3" fontWeight={700}>
                      {showBalance 
                        ? `$${portfolio.totalBalance.usd.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` 
                        : '••••••'
                      }
                    </Typography>
                  </motion.div>
              ...
              <Box display="flex" alignItems="center" gap={1} mt={1}>
                <Typography variant="body2" sx={{ opacity: 0.8 }}>
                  {showBalance ? `${portfolio.totalBalance.eth.toFixed(4)} ETH` : '•••• ETH'}
                </Typography>
                <Box 
                  display="flex" 
                  alignItems="center" 
                  gap={0.5}
                  sx={{ color: getChangeColor(portfolio.ethBalance.change24h) }}
                >
                  {getChangeIcon(portfolio.ethBalance.change24h)}
                  <Typography variant="caption" fontWeight={600}>
                    {Math.abs(portfolio.ethBalance.change24h).toFixed(2)}%
                  </Typography>
                </Box>
              </Box>
            </Box>
          ...
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Box display="flex" alignItems="center" gap={1}>
              ...
              >
                <IconButton
                  size="small"
                  onClick={() => copyToClipboard(portfolio.walletAddress)}
                  sx={{ color: 'white' }}
                >
                  <Copy size={14} />
                </IconButton>
