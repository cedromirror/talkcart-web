...
const SendDialog: React.FC<SendDialogProps> = ({ open, onClose, selectedToken }) => {
  ...
  const { portfolio, estimateGas, sendTransaction, loading } = useWallet();
  ...
  const availableTokens = portfolio ? [
    {
      symbol: 'ETH',
      name: 'Ethereum',
      balance: portfolio.ethBalance.amount,
      logo: '⟠',
    },
                ...
                >
                  {availableTokens.map((token) => (
                    <MenuItem key={token.symbol} value={token.symbol}>
                      <Box display="flex" alignItems="center" gap={2}>
                        <Typography fontSize="1.2rem">{token.logo}</Typography>
                        <Box>
                    ...
                    <>
                      <Box display="flex" justifyContent="space-between" mb={1}>
                        ...
                        <Typography variant="body2">
                          {gasEstimate.totalCost.toFixed(6)} ETH
                        </Typography>
                      </Box>
                      ...
                      <Box display="flex" justifyContent="space-between">
                        ...
                        <Typography variant="body2">
                          ~${gasEstimate.totalCostUsd.toFixed(2)}
                        </Typography>
                      </Box>
                    </>
                  )}
