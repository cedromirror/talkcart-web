...
export class BiometricPlatformService {
  ...
  public async initialize(deviceInfo?: any): Promise<void> {
    ...
    if (device.isIOS) {
      this.platformConfig = PLATFORM_CONFIGS.ios;
    } else if (device.isAndroid) {
      this.platformConfig = PLATFORM_CONFIGS.android;
    } else if (device.isWindows) {
      this.platformConfig = PLATFORM_CONFIGS.windows;
    } else if (device.isMacOS) {
      this.platformConfig = PLATFORM_CONFIGS.macos;
    } else {
      this.platformConfig = PLATFORM_CONFIGS.other;
    }

    // Determine browser quirks
    this.browserQuirks = BROWSER_QUIRKS[device.browser] || BROWSER_QUIRKS.unknown;
  ...
  public async performPreChecks(): Promise<{ success: boolean; errors: string[] }> {
    ...
    if (this.browserQuirks?.hasUserActivationBug) {
      ...
      const userActivated = document.hasStoredUserActivation || 
