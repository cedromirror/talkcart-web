...
export function useStreamData(streamId: string) {
  ...
  const stream = streamResponse?.data as StreamData | undefined;

  // Fetch stream metrics
  const { data: metricsResponse } = useQuery({
    queryKey: ['stream-metrics', streamId],
    queryFn: () => api.streams.getStreamMetrics(streamId),
  ...
  const metrics = metricsResponse?.data as StreamMetrics | undefined;

  // Fetch stream health (for streamers)
  const { data: healthResponse } = useQuery({
    queryKey: ['stream-health', streamId],
    queryFn: () => api.streams.getHealth(streamId),
  ...
  const health = healthResponse?.data as StreamHealth | undefined;
  ...
  const followMutation = useMutation({
    mutationFn: (streamerId: string) => api.streams.followStreamer(streamerId),
    onSuccess: () => {
  ...
  const unfollowMutation = useMutation({
    mutationFn: (streamerId: string) => api.streams.unfollowStreamer(streamerId),
    onSuccess: () => {
  ...
  const likeMutation = useMutation({
    mutationFn: (streamId: string) => api.streams.likeStream(streamId),
    onSuccess: () => {
  ...
  const reportMutation = useMutation({
    mutationFn: ({ streamId, reason }: { streamId: string; reason: string }) =>
      api.streams.reportStream(streamId, reason),
    onSuccess: () => {
