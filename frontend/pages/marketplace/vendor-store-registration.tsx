  ...
  const handleSave = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (response && response.success) {
        ...
        // If this is a new store creation, refresh the user profile to update the role
        if (!storeExists && user) {
          try {
            // Small delay to ensure backend has processed the role change
            await new Promise(resolve => setTimeout(resolve, 3000));
            
            // Refresh user profile to get updated role with force refresh
        ...
        // Always redirect to vendor dashboard after successful save
