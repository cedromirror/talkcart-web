...
export default function PaymentsAdmin() {
  ...
  }, [guard.allowed, tabValue, after, before, limit, statusFilter, customerFilter, timeRangeFilter]);
                  ...
                  <TableBody>
                    {items.map((payment) => (
                      <TableRow key={payment.id}>
                        ...
                        <TableCell>
                          <Typography variant="body2">
                            {payment.payment_method_types?.join(', ') || 'Unknown'}
                          </Typography>
                        </TableCell>
                        <TableCell>
          ...
          <Card>
            <CardContent>
              ...
              <Typography variant="body2" color="textSecondary" gutterBottom>
                Create a test Stripe PaymentIntent to verify connectivity and functionality.
              </Typography>
                ...
                <CardContent>
                  ...
                  <Stack spacing={1}>
                    ...
                    <Box display="flex" justifyContent="space-between">
                      ...
                      <Chip
                        label={selectedPayment.payment?.status?.replace('_', ' ')}
                    ...
                    <Box display="flex" justifyContent="space-between">
                      ...
                      <Typography variant="body2">{selectedPayment.payment?.currency?.toUpperCase()}</Typography>
                    </Box>
                    <Box display="flex" justifyContent="space-between">
                      <Typography variant="body2" color="textSecondary">Customer:</Typography>
                      <Typography variant="body2">{selectedPayment.payment?.customer || 'Guest'}</Typography>
                    </Box>
                    <Box display="flex" justifyContent="space-between">
                      <Typography variant="body2" color="textSecondary">Payment Method:</Typography>
                      <Typography variant="body2">
                        {selectedPayment.payment?.payment_method_types?.join(', ') || 'Not available'}
                      </Typography>
                    </Box>
                    <Box display="flex" justifyContent="space-between">
                      ...
                      {selectedPayment.charges.map((charge: any) => (
                        <Box key={charge.id} p={1} border={1} borderColor="grey.300" borderRadius={1}>
                          ...
                          <Typography variant="caption" color="textSecondary">
                            {charge.status} • {formatCurrency(charge.amount, charge.currency)}
                          </Typography>
                        </Box>
                      ))}
                    </Stack>
