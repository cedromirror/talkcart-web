  ...
  try {
    ...
    if (created) {
      try {
        ...
        listParams.created = created; // Fallback raw if parsing fails
      }
    }
...
router.get('/vendors/:id/fees', authenticateTokenStrict, requireAdmin, async (req, res) => {
  try {
    ...
    // Calculate platform fees from completed orders
    const feesData = await Order.aggregate([
      ...
      },
      {
        $group: {
          ...
          platformFees: { $sum: { $multiply: ['$total', 0.05] } }, // 5% platform fee
